...
UPDATEBOT_NFS_MODIFY_FILES := $(shell git diff --name-only $${GIT_LAST_TAG:-HEAD~1} roles/v1alpha1/nfs 2>/dev/null | wc -l )
UPDATEBOT_KEYDB_MODIFY_FILES := $(shell git diff --name-only $${GIT_LAST_TAG:-HEAD~1} roles/v1alpha1/database/keydb 2>/dev/null | wc -l )
UPDATEBOT_POSTGRES_MODIFY_FILES := $(shell git diff --name-only $${GIT_LAST_TAG:-HEAD~1} roles/v1alpha1/database/postgres 2>/dev/null | wc -l )
